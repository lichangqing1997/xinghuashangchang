  ...
  <view class="scan-page">
    ...
    <view v-if="product" class="card product-card">
      <view class="product-header">
        ...
        <uni-tag :text="product.类别" type="warning" size="small" />
      </view>
    ...
    <view class="card form-card">
      ...
      <view class="form-item">
        <text class="form-label">销售单号</text>
        <input
          v-model="form.销售单号"
          placeholder="可选，关联销售单"
          class="form-input"
        />
      </view>
...
async function handleSubmit() {
  ...
  try {
    const msg = await scanOut({
      条形码: form.条形码.trim(),
      数量: form.数量,
      销售单号: form.销售单号 || undefined,
      操作人: form.操作人 || undefined
    })
    uni.showToast({ title: msg || '出库成功', icon: 'success' })
    // 重置表单
    form.条形码 = ''
    form.数量 = 1
    form.销售单号 = ''
...
.scan-btn {
  ...
  box-shadow: 0 8rpx 24rpx rgba(243, 167, 63, 0.3);
}
